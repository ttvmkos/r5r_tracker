...
{	
	...
	if ( p_damage == 2 && victimhealth <= 96 )
	{
		...
		if ( IsValid(attacker) && attacker.IsPlayer() && IsAlive(attacker) ){
			...
			EmitSoundOnEntityOnlyToPlayer( attacker, attacker, attacker.p.hitsound )
			
			float atthealth = attacker.GetHealth().tofloat();
...
void function SetupInfiniteAmmoForWeapon( entity player, entity weapon)
{
	if( IsValid( weapon ) && weapon.UsesClipsForAmmo() )
	{
		...
		player.AmmoPool_SetCount( ammoType, 65535 ) //mkosDEBUG
